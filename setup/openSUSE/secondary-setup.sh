...
bash "$SYSTEMD_SCRIPT" || { echo "Error: Failed to run $SYSTEMD_SCRIPT (exit code $?)"; exit 1; }

if [ "$WITH_BOINC" = true ]; then
    echo "DEBUG: Executing BOINC setup with $BOINC_SCRIPT"
    echo "Running BOINC setup..."
    bash "$BOINC_SCRIPT" || { echo "Error: Failed to run $BOINC_SCRIPT (exit code $?)"; exit 1; }
fi
...
if [ "$WITH_DISPLAY_IDLE" = true ] || [ "$WITH_LID_CONTROL" = true ]; then
    ...
    bash "$DISPLAY_SCRIPT" \
        ...
        $([ "$WITH_LID_CONTROL" = true ] && echo "--with-lid-control") || { echo "Error: Failed to run $DISPLAY_SCRIPT (exit code $?)"; exit 1; }
fi

if [ "$WITH_SWAY" = true ]; then
    echo "DEBUG: Executing Sway setup with $SWAY_SCRIPT"
    echo "Running Sway window manager setup..."
    bash "$SWAY_SCRIPT" || { echo "Error: Failed to run $SWAY_SCRIPT (exit code $?)"; exit 1; }
fi

if [ "$WITH_OPENBOX" = true ]; then
    echo "DEBUG: Executing Openbox setup with $OPENBOX_SCRIPT"
    echo "Running Openbox window manager setup..."
    bash "$OPENBOX_SCRIPT" || { echo "Error: Failed to run $OPENBOX_SCRIPT (exit code $?)"; exit 1; }
fi
